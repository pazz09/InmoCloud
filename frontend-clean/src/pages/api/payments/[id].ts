...
import { deletePayment, updatePayment } from "@/lib/backend/payments";
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (req.method === "PUT")
    try {
      ...
      try {
        const response = deletePayment(id);
        return res.status(200).json(SuccessTemplate(response, "Información eliminada con éxito"))
      } catch (err) {
        if (err instanceof z.ZodError) {
          console.log(err)
          return handleZodError(err, res) ;
